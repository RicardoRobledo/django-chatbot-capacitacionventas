...
let base_url = 'https://django-chatbot-capacitacionventas.onrender.com/api/';
...
<div class="conversation-thread thread-ai" id="welcome-message">
  ${avatar_in_chat}
	...
	<div class="user-name"><h5>${employee_name}</h5></div>
	  <div class="message-text">
		<div class="chat-response">¿Que tal ${$("#badge").find("#badge_text").text()}?, Preguntame lo que necesites saber</div>
	  </div>
		<div class="date-chat"><img src="../../static/img/icon-clock.svg"> ${current_date}</div>
	</div>
  </div>
...
`);
	...
	$(".ai-contacts-top input[type='text']").on("input", function(event) {
		...
		for (let json of data_procedures) {
